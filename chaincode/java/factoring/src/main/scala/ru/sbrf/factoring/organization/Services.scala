...
import com.github.apolubelov.fabric.contract.store.KeyValue
...
trait Services {
  ...
  @ContractOperation
  def removeOrganizations(context: ContractContext, id: String): ContractResponse = {
    context.store.del(id)
    Success()
  }
  ...
  @ContractOperation
  def listOrganizationsWithKeys(context: ContractContext): ContractResponse = {
    //    val myId = context.clientIdentity.mspId
    val organizations: Array[KeyValue[Organization]] = context.store.list[Organization]
      //      .map(_.value) // take only values
      .toArray // use Array, as GSON knows nothing about scala collections
